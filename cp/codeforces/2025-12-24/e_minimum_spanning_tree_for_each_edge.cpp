...
class LCA {
      private:
        ...
        void preDFS(int node, int par, int wt) {
                ...
                for (int i = 1; i < LOG; i++) {
                        int mid = ancs[node][i - 1];
                        if (mid != -1) {
                                ancs[node][i] = ancs[mid][i - 1];
                                mxEdge[node][i] = max(mxEdge[node][i - 1], mxEdge[mid][i - 1]);
                        }
                }
      ...
      public:
        ...
        int getMxWtInTreePath(int a, int b) {
...
void solve( ) {
        ...
        for (auto &it : edges) {
                int heaviest = lca.getMxWtInTreePath(it.u, it.v);
                res[it.idx] = mstWt - heaviest + it.w; // to minimize spanning tree wt
        }
